...
3. 有新版时点「更新到最新版」（自动重新下载 + 编译原生模块，需几分钟）
4. **重启应用**生效

命令行方式（跳过界面）：

```bash
# 删除已下载的运行时，下次启动自动重新下载最新版
rm -rf ~/.config/Electron/dsh-runtime
```

应用本体（dsh-desktop）的更新：改 `package.json` 版本号 → `git tag vX.Y.Z && git push origin vX.Y.Z`，CI 自动构建并发布新安装包。

## 配置优先级

**环境变量 > 设置文件 > 默认值**。下表为环境变量（优先级最高，适合脚本化/自动化部署）：
